...
public sealed partial class StockReceiptController(IConfiguration configuration, ILogger<StockReceiptController>? logger = null) : ControllerBase
{
    ...
    public async Task<IActionResult> Lookup(CancellationToken token)
    {
        ...
        var items=await Rows(c,$"SELECT I.ItemID,I.ItemCode,I.ItemName,I.StockTrackingCode,I.UnitCode FROM dbo.TDIVItem I WHERE I.CompanyID=@company AND I.IsActive=1 AND I.ItemKindCode=N'GOODS' AND I.StockTrackingCode<>N'NONE' AND {ItemProjectAccess.ItemAliasPredicate} ORDER BY I.ItemCode",token,r=>new{itemID=r.GetInt64(0),itemCode=r.GetString(1),itemName=r.GetString(2),stockTrackingCode=r.GetString(3),unitCode=r.GetString(4)});
    ...
    [HttpGet]
    public async Task<IActionResult> List([FromQuery]string? search,[FromQuery]string? status,CancellationToken token)
    {
        await using var c=await Open(token); if(!await Can(c,"VIEW",token))return Forbid();
        var sql=$"SELECT R.StockReceiptID,R.ReceiptCode,R.ReceiptDate,R.ReceiptType,R.StatusCode,R.ReferenceNo,W.WarehouseCode,W.WarehouseName,COALESCE(SUM(D.Quantity),0) TotalQuantity FROM dbo.TDIVStockReceipt R JOIN dbo.TDIVWarehouse W ON W.WarehouseID=R.WarehouseID AND W.CompanyID=R.CompanyID INNER JOIN dbo.TDADBranch B ON B.BranchID=W.BranchID AND B.CompanyID=W.CompanyID AND B.IsActive=1 LEFT JOIN dbo.TDIVStockReceiptDetail D ON D.StockReceiptID=R.StockReceiptID WHERE R.CompanyID=@company AND R.IsActive=1 AND {WarehouseAccessService.WarehouseAliasPredicate} AND (@status=N'' OR R.StatusCode=@status) AND (@search=N'' OR R.ReceiptCode LIKE @like OR R.ReferenceNo LIKE @like) GROUP BY R.StockReceiptID,R.ReceiptCode,R.ReceiptDate,R.ReceiptType,R.StatusCode,R.ReferenceNo,W.WarehouseCode,W.WarehouseName ORDER BY R.ReceiptDate DESC,R.StockReceiptID DESC";
        await using var cmd=new SqlCommand(sql,c);var q=search?.Trim()??"";Add(cmd,"@company",SqlDbType.BigInt,CompanyId());Add(cmd,"@user",SqlDbType.BigInt,UserId());Add(cmd,"@status",SqlDbType.NVarChar,status?.Trim().ToUpperInvariant()??"",20);Add(cmd,"@search",SqlDbType.NVarChar,q,200);Add(cmd,"@like",SqlDbType.NVarChar,$"%{q}%",210);
        var rows=new List<object>();await using var reader=await cmd.ExecuteReaderAsync(token);while(await reader.ReadAsync(token))rows.Add(new{stockReceiptID=reader.GetInt64(0),receiptCode=reader.GetString(1),receiptDate=reader.GetDateTime(2),receiptType=reader.GetString(3),statusCode=reader.GetString(4),referenceNo=Text(reader,5),warehouseCode=reader.GetString(6),warehouseName=reader.GetString(7),totalQuantity=reader.GetDecimal(8)});return Ok(rows);
    }
    ...
    public async Task<IActionResult> Get(long id,CancellationToken token)
    {
        ...
        const string detailSql="SELECT D.StockReceiptDetailID,D.[LineNo],D.ItemID,I.ItemCode,I.ItemName,I.StockTrackingCode,D.Quantity,D.UnitCost,D.Remark,D.SerialSourceCode,I.UnitCode FROM dbo.TDIVStockReceiptDetail D JOIN dbo.TDIVItem I ON I.ItemID=D.ItemID WHERE D.StockReceiptID=@id AND I.CompanyID=@company ORDER BY D.[LineNo]";
        await using var detail=new SqlCommand(detailSql,c);Add(detail,"@id",SqlDbType.BigInt,id);Add(detail,"@company",SqlDbType.BigInt,CompanyId());var items=new List<object>();await using var dr=await detail.ExecuteReaderAsync(token);while(await dr.ReadAsync(token))items.Add(new{stockReceiptDetailID=dr.GetInt64(0),lineNo=dr.GetInt32(1),itemID=dr.GetInt64(2),itemCode=dr.GetString(3),itemName=dr.GetString(4),stockTrackingCode=dr.GetString(5),quantity=dr.GetDecimal(6),unitCost=dr.GetDecimal(7),remark=Text(dr,8),serialSourceCode=dr.GetString(9),unitCode=dr.GetString(10)});await dr.DisposeAsync();
    ...
    public async Task<IActionResult> Confirm(long id,CancellationToken token)
    {
        ...
        try
        {
            ...
            foreach(var line in lines)
            {
                ...
                foreach(var serial in serials)await CreateInstance(c,tx,warehouse.Value,line.Item,serial,id,line.Detail,token);
            }
