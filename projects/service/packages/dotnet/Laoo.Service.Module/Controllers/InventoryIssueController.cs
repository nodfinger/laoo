...
public sealed class InventoryIssueController(IConfiguration configuration):ControllerBase
{
    ...
    public async Task<IActionResult> List(CancellationToken token)
    {
        ...
        var sql=$"SELECT S.StockIssueID,S.IssueCode,S.IssueDate,S.WorkOrderCode,S.StatusCode,S.Remark FROM dbo.TDIVStockIssue S INNER JOIN dbo.TDIVWarehouse W ON W.WarehouseID=S.WarehouseID AND W.CompanyID=S.CompanyID INNER JOIN dbo.TDADBranch B ON B.BranchID=W.BranchID AND B.CompanyID=W.CompanyID AND B.IsActive=1 WHERE S.CompanyID=@company AND S.IsActive=1 AND {WarehouseAccessService.WarehouseAliasPredicate} ORDER BY S.IssueDate DESC,S.StockIssueID DESC";await using var cmd=new SqlCommand(sql,c);Add(cmd,"@company",SqlDbType.BigInt,CompanyId());Add(cmd,"@user",SqlDbType.BigInt,UserId());var rows=new List<object>();await using var r=await cmd.ExecuteReaderAsync(token);while(await r.ReadAsync(token))rows.Add(new{stockIssueID=r.GetInt64(0),issueCode=r.GetString(1),issueDate=r.GetDateTime(2),workOrderCode=r.GetString(3),statusCode=r.GetString(4),remark=Text(r,5)});return Ok(rows);
    }
    ...
    public async Task<IActionResult> Lookup(CancellationToken token)
    {
        ...
        await using(var cmd=new SqlCommand($"SELECT W.WarehouseID,W.WarehouseCode,W.WarehouseName,W.IsDefault FROM dbo.TDIVWarehouse W INNER JOIN dbo.TDADBranch B ON B.BranchID=W.BranchID AND B.CompanyID=W.CompanyID AND B.IsActive=1 WHERE W.CompanyID=@company AND W.IsActive=1 AND {WarehouseAccessService.WarehouseAliasPredicate} ORDER BY W.IsDefault DESC,W.WarehouseCode",c)){Add(cmd,"@company",SqlDbType.BigInt,CompanyId());Add(cmd,"@user",SqlDbType.BigInt,UserId());await using var r=await cmd.ExecuteReaderAsync(token);while(await r.ReadAsync(token))warehouses.Add(new{warehouseID=r.GetInt64(0),warehouseCode=r.GetString(1),warehouseName=r.GetString(2),isDefault=r.GetBoolean(3)});}
        await using(var cmd=new SqlCommand($"SELECT I.ItemID,I.ItemCode,I.ItemName,I.StockTrackingCode FROM dbo.TDIVItem I WHERE I.CompanyID=@company AND I.IsActive=1 AND I.ItemKindCode=N'GOODS' AND EXISTS(SELECT 1 FROM dbo.TDIVItemUsage U WHERE U.CompanyID=I.CompanyID AND U.ItemID=I.ItemID AND U.UsageCode IN(N'MATERIAL',N'SPARE_PART')) AND {ItemProjectAccess.ItemAliasPredicate} ORDER BY I.ItemCode",c)){Add(cmd,"@company",SqlDbType.BigInt,CompanyId());await using var r=await cmd.ExecuteReaderAsync(token);while(await r.ReadAsync(token))items.Add(new{itemID=r.GetInt64(0),itemCode=r.GetString(1),itemName=r.GetString(2),stockTrackingCode=r.GetString(3)});}
        await using(var cmd=new SqlCommand($"SELECT X.ItemInstanceID,X.ItemID,X.WarehouseID,X.SerialNo FROM dbo.TDIVItemInstance X INNER JOIN dbo.TDIVItem I ON I.ItemID=X.ItemID AND I.CompanyID=X.CompanyID INNER JOIN dbo.TDIVWarehouse W ON W.WarehouseID=X.WarehouseID AND W.CompanyID=X.CompanyID INNER JOIN dbo.TDADBranch B ON B.BranchID=W.BranchID AND B.CompanyID=W.CompanyID AND B.IsActive=1 WHERE X.CompanyID=@company AND X.StatusCode=N'IN_STOCK' AND {WarehouseAccessService.WarehouseAliasPredicate} AND {ItemProjectAccess.ItemAliasPredicate} ORDER BY X.SerialNo",c)){Add(cmd,"@company",SqlDbType.BigInt,CompanyId());Add(cmd,"@user",SqlDbType.BigInt,UserId());await using var r=await cmd.ExecuteReaderAsync(token);while(await r.ReadAsync(token))serials.Add(new{itemInstanceID=r.GetInt64(0),itemID=r.GetInt64(1),warehouseID=r.GetInt64(2),serialNo=r.GetString(3)});}
    ...
    public async Task<IActionResult> Create([FromBody]InventoryIssueRequest request,CancellationToken token)
    {
        ...
        try
        {
            if(!await WarehouseAccessService.CanAccessAsync(c,tx,CompanyId(),UserId(),request.WarehouseID,token)){await tx.RollbackAsync(token);return StatusCode(403,new{message="ไม่มีสิทธิ์เข้าถึงคลัง",description="คลังไม่อยู่ในสิทธิ์ของผู้ใช้หรือปิดใช้งาน"});}
        ...
        catch(Exception ex) when (ex is not ItemProjectDeniedException){await tx.RollbackAsync(token);return BadRequest(new{message="บันทึกใบเบิกจ่ายไม่สำเร็จ",description=ex.Message});}
    }
    ...
    private async Task<IActionResult> ChangeStatus(long id,bool confirm,CancellationToken token)
    {
        ...
        try
        {
            ...
            {
                if(confirm) await ItemProjectAccess.EnsureAsync(c,tx,CompanyId(),line.Item,token);
        ...
        catch(Exception ex) when (ex is not ItemProjectDeniedException){await tx.RollbackAsync(token);return BadRequest(new{message="เปลี่ยนสถานะใบเบิกจ่ายไม่สำเร็จ",description=ex.Message});}
    }
