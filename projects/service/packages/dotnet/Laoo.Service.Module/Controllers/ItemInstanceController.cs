...
""";
    ...
    public async Task<IActionResult> UpdateLocation(long id,ItemInstanceLocationRequest request,CancellationToken token)
    {
        ...
        try
        {
            await using var current=new SqlCommand("SELECT StatusCode FROM dbo.TDIVItemInstance WITH(UPDLOCK,HOLDLOCK) WHERE ItemInstanceID=@id AND CompanyID=@company",c,tx);Add(current,"@id",SqlDbType.BigInt,id);Add(current,"@company",SqlDbType.BigInt,CompanyId());var from=Convert.ToString(await current.ExecuteScalarAsync(token));if(string.IsNullOrEmpty(from))return NotFound();if(request.BranchID.HasValue){await using var branch=new SqlCommand("SELECT CASE WHEN EXISTS(SELECT 1 FROM dbo.TDADBranch WHERE BranchID=@branch AND CompanyID=@company) THEN 1 ELSE 0 END",c,tx);Add(branch,"@branch",SqlDbType.BigInt,request.BranchID);Add(branch,"@company",SqlDbType.BigInt,CompanyId());if(Convert.ToInt32(await branch.ExecuteScalarAsync(token))!=1)return BadRequest(new{message="สาขาไม่ถูกต้อง",description="สาขาต้องอยู่ภายใน Company เดียวกัน"});}
        ...
        catch(Exception ex){try{await tx.RollbackAsync(token);}catch{}return BadRequest(new{message="อัปเดต Serial ไม่สำเร็จ",description=ex.Message});}
    }
