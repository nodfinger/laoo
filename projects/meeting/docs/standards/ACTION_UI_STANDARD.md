...
- Caption ใช้รูปแบบ `{MenuName} > {Action}` และค่ากลาง ขนาด `18px`, `FontWeight.w700`, สีดำ `LaooColors.pageCaption` ห้ามกำหนด Style แยกรายหน้า
- ปุ่ม `ยกเลิก` และ `บันทึก` อยู่ขวาของ Caption ในบรรทัดเดียวกันเมื่อพื้นที่เพียงพอ
- เมื่อ Content Area กว้างน้อยกว่า `700px` ให้ย้ายชุดปุ่ม Action ลงบรรทัดใหม่ใต้ Caption และจัดชิดซ้ายเสมอ
- หน้า Action ต้องใช้ `WorkspaceActionHeader` หรือ Component กลางที่ให้พฤติกรรมเดียวกัน ห้ามใช้ `Row` ตายตัวสำหรับ Caption กับปุ่ม
...
- ปุ่มใช้ Font `14px`, สูง `48px` จาก `LaooTypography.buttonHeight` และจัด Icon/ข้อความกึ่งกลางแนวตั้ง
