...
- Caption ใช้ Style กลาง `LaooTypography.popupTitleStyle`: ขนาด `18px`, `FontWeight.w700`, สีดำ `LaooColors.pageCaption`; Icon หน้า Caption ใช้ Primary ตาม User Style
...
- ปุ่มด้านล่างใช้ Font `14px`, สูง `48px`, มุมโค้ง `4px`; ยกเลิกใช้ Text/Outlined Primary และบันทึกใช้ Filled Primary
...
- ใช้ Pattern เดียวกันทั้งระบบ: Icon ถังขยะสีแดง, Caption ยังคงเป็นสีดำ `18px` และ `FontWeight.w700` ตามมาตรฐานกลาง, กล่องข้อความพื้นแดงอ่อนที่แสดง Key/ชื่อรายการ และข้อความว่าเรียกคืนไม่ได้
...
```text
ปรับ Popup/Dialog/Alert นี้ตาม docs/standards/UX_UI_STANDARD.md, POPUP_UI_STANDARD.md และ TYPOGRAPHY_STANDARD.md เอาเส้นกรอบรอบนอกออก Caption สีดำ 18px FontWeight.w700 รวม Delete Confirm, Icon ใช้สี Primary และ Icon ลบใช้สีแดง มีเส้นเทาอ่อนใต้ Caption และก่อนปุ่ม TextBox/ComboBox มุมโค้ง 4px Validation สีแดงใต้ Field ปุ่มด้านล่างสูง 48px มุมโค้ง 4px และใช้สี User Style Notification ต้องลอยมุมขวาบนและหายตาม TimeAlert ห้ามแก้ API, SQL, Repository หรือ Business Logic ตรวจ Responsive, Overflow, dart format และ dart analyze
```
