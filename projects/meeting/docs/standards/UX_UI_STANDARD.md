...
ไฟล์นี้เป็นมาตรฐาน UX/UI กลางของ Project `laoo_meeting` และเป็นจุดเริ่มต้นก่อนอ่านมาตรฐานเฉพาะประเภทหน้าจอ
...
- Caption ด้านบนของหน้าจอทุกประเภท (`List`, `Card`, `Action`, `Popup/Dialog`) ต้องใช้มาตรฐานเดียวกัน: `fontSize: 18px`, `color: LaooColors.pageCaption` (สีดำ) และ `fontWeight: FontWeight.w700`
- Caption ต้องอ่านค่าจาก `LaooTypography.workspaceCaption`, `LaooTypography.workspaceCaptionWeight`, `LaooColors.pageCaption` หรือ Style กลางที่ประกอบจากค่าเหล่านี้ ห้ามกำหนดขนาด สี หรือน้ำหนักแยกในแต่ละหน้าจอ
- Icon หน้า Caption ใช้สีหลักของ User Style
...
```text
ปรับหน้าจอใน Project C:\laooplatform\laoo_meeting โดยอ่าน AGENTS.md และ docs/standards/UX_UI_STANDARD.md ก่อน จากนั้นอ่านมาตรฐานเฉพาะประเภทหน้าจอที่เกี่ยวข้อง ห้ามแก้ API, SQL, Repository หรือ Business Logic ให้ใช้ LaooColors, LaooLayout, LaooRadius, LaooTypography และ Workspace Theme จากส่วนกลางเท่านั้น ตรวจ Permission, Responsive, Overflow, dart format และ dart analyze แล้วสรุปไฟล์ที่แก้
```
