...
```text
C:\laooplatformplatformplatform\laoo\laoo_meeting_api
```
...
```text
laoo_meeting_api.csproj
```

ก่อนวางทับ แนะนำให้สำรองโฟลเดอร์เดิม และเก็บ `local.json` ไว้นอก ZIP

```text
LaooMeetingApi.csproj
laoo_meeting_api.csproj เดิม
```
...
```powershell
Rename-Item C:\laooplatformplatformplatform\laoo\laoo_meeting_api C:\laooplatformplatformplatform\laoo\laoo_meeting_api_backup
```

2. สร้างโฟลเดอร์ใหม่และแตก ZIP ลงไป:

```powershell
New-Item -ItemType Directory C:\laooplatformplatformplatform\laoo\laoo_meeting_api
```
...
```powershell
cd C:\laooplatformplatformplatform\laoo\laoo_meeting_api
dotnet run --project .\laoo_meeting_api.csproj
```
...
DataProtection__ApplicationName=LaooMeetingApi
