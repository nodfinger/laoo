...
""", connection);
    ...
    public async Task<IActionResult> Update([FromBody] UserProfileUpdate request, CancellationToken token)
    {
        ...
        var menuStyle = string.Equals(request.MenuStyleCode?.Trim(), "BUTTON", StringComparison.OrdinalIgnoreCase)
            ? "BUTTON"
            : "SLIDE";
        var defaultViewMode = string.Equals(request.DefaultViewMode?.Trim(), "CARD", StringComparison.OrdinalIgnoreCase)
            ? "CARD"
            : "LIST";
        byte[]? avatar = null;
...
UPDATE dbo.TDADUserProfile SET AvatarData=CASE WHEN @remove=1 THEN NULL ELSE COALESCE(@avatar,AvatarData) END,AvatarContentType=CASE WHEN @remove=1 THEN NULL ELSE COALESCE(@type,AvatarContentType) END,AvatarFileName=CASE WHEN @remove=1 THEN NULL ELSE COALESCE(@file,AvatarFileName) END,ThemeCode=@theme,MenuStyleCode=@menuStyle,DefaultViewMode=@defaultViewMode,Introduction=@intro,UpdateDate=SYSUTCDATETIME()
WHERE {owner.Value.ProfileKey}=@id;
IF @@ROWCOUNT=0 INSERT dbo.TDADUserProfile(UserType,{owner.Value.ProfileKey},AvatarData,AvatarContentType,AvatarFileName,ThemeCode,MenuStyleCode,DefaultViewMode,Introduction) VALUES(@userType,@id,@avatar,@type,@file,@theme,@menuStyle,@defaultViewMode,@intro);
""", connection, transaction);
            ...
            profile.Parameters.Add("@avatar", SqlDbType.VarBinary, -1).Value = (object?)avatar ?? DBNull.Value; Add(profile, "@type", SqlDbType.NVarChar, request.AvatarContentType, 100); Add(profile, "@file", SqlDbType.NVarChar, request.AvatarFileName, 250); Add(profile, "@theme", SqlDbType.NVarChar, request.ThemeCode, 30); Add(profile, "@menuStyle", SqlDbType.NVarChar, menuStyle, 10); Add(profile, "@defaultViewMode", SqlDbType.NVarChar, defaultViewMode, 10); Add(profile, "@intro", SqlDbType.NVarChar, request.Introduction, 1000);
...
public sealed record UserProfileUpdate(string? Username, string? CurrentPassword, string? NewPassword, string? ThemeCode, string? MenuStyleCode, string? DefaultViewMode, string? Introduction, string? AvatarDataBase64, string? AvatarContentType, string? AvatarFileName, bool RemoveAvatar = false);
