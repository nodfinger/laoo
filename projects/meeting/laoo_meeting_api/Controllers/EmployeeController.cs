...
public sealed class EmployeeController(IConfiguration configuration, PasswordService passwordService) : ControllerBase
{
        ...
        CancellationToken token = default)
    {
        ...
        const string sql = """
            SELECT COUNT_BIG(1) OVER() AS TotalCount,
                   ...
                   ,E.NotifyByEmail,E.NotifyBySystem
    ...
    private async Task<IActionResult> Save(long? id, EmployeeUpsertRequest x, CancellationToken token)
    {
        ...
        if (!x.NotifyByEmail && !x.NotifyBySystem)
            return BadRequest(new { message = "กรุณาเลือกวิธีแจ้งเตือนอย่างน้อย 1 วิธี" });
        if (x.NotifyByEmail && string.IsNullOrWhiteSpace(x.Email))
            return BadRequest(new { message = "กรุณาระบุ Email เมื่อเลือกแจ้งเตือนทาง Email" });
        ...
        cmd.Parameters.Add("@notifyEmail", SqlDbType.Bit).Value = x.NotifyByEmail;
        cmd.Parameters.Add("@notifySystem", SqlDbType.Bit).Value = x.NotifyBySystem;
...
""";
    ...
    private static EmployeeResponse Read(SqlDataReader r) => new()
    {
        ...
        StartWorkDate = r.IsDBNull(21) ? null : r.GetDateTime(21), HasImage = r.GetInt32(22) != 0, IsActive = r.GetBoolean(23), CarID1 = NString(r, 24), CarColor1 = NString(r, 25), CarTypeCode1 = NString(r, 26), CarOilType1 = NString(r, 27), CarID2 = NString(r, 28), CarColor2 = NString(r, 29), CarTypeCode2 = NString(r, 30), CarOilType2 = NString(r, 31), NotifyByEmail = r.GetBoolean(32), NotifyBySystem = r.GetBoolean(33)
    };
