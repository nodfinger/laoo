...
class _EmployeeUxPageState extends State<EmployeeUxPage> {
  ...
  String get _menuKey => widget.companyScoped
      ? '10001'
      : widget.customer
      ? '12001'
      : '11001';
  ...
  Future<void> _loadRoleGroups() async {
    try {
      final groups = await RoleGroupRepository().list(
        widget.customer || widget.companyScoped ? 'customer' : 'partner',
      );
      if (mounted) {
        setState(() => _roleGroups = groups.where((e) => e.isActive).toList());
      }
    } catch (_) {}
  }
  ...
  Future<void> _pickFormalImage() async {
    ...
    try {
      ...
      if (decoded == null) {
        throw StateError('ไม่สามารถอ่านไฟล์รูปภาพได้');
      }
      var quality = 85;
      ...
      if (mounted) {
        setState(() {
          _formalImageBytes = compressed;
          _formalImageName = picked!.files.single.name;
        });
      }
    } catch (error) {
      if (mounted) {
      ...
      if (mounted) {
        setState(() => _formalImageBusy = false);
      }
    }
  }
  ...
  Future<void> _pickCarImage(int carNo) async {
    ...
    try {
      final decoded = img.decodeImage(source);
      if (decoded == null) {
        throw StateError('ไม่สามารถอ่านไฟล์รูปรถได้');
      }
      var quality = 85;
      Uint8List compressed = Uint8List.fromList(
        img.encodeJpg(decoded, quality: quality),
  ...
  Future<void> _loadEmployees() async {
    ...
    try {
      ...
      setState(() {
        ...
        rows = loadedRows;
        _totalCount =
            filterIsActive != null && activeFilteredItems.length != items.length
  ...
  List<Map<String, dynamic>> get filterDepartments =>
      organizationUnits.where((unit) {
        if (unit['unitType'] != 'DEP' || _unitId(unit) == null) {
          return false;
        }
  ...
  Future<void> _clearEmployeeFilters() async {
    setState(() {
      searchController.clear();
      filterDivisionId = null;
      filterDepartmentId = null;
  ...
  Widget build(BuildContext context) => SupportWorkspaceShell(
    pageTitle: 'พนักงาน',
    activeMenu: widget.companyScoped
        ? 'companyEmployees'
        : widget.customer
        ? 'customerEmployees'
        : 'partnerEmployees',
  ...
  Widget _employeeList() => Column(
    ...
    children: [
      ...
      WorkspaceSectionCard(
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            final standardFieldWidth = compact ? constraints.maxWidth : 190.0;
            final statusFieldWidth = compact ? constraints.maxWidth : 160.0;
            final customerFieldWidth = compact ? constraints.maxWidth : 240.0;
            return Wrap(
              spacing: 8,
              runSpacing: 8,
              ...
              children: [
                ...
                OutlinedButton.icon(
                  ...
                  onPressed: _clearEmployeeFilters,
      ...
      Flexible(
        ...
        child: ValueListenableBuilder<WorkspaceThemePreset>(
          valueListenable: workspaceThemeController,
          builder: (_, _, _) => _employeeTableStyle(),
        ),
      ),
                ...
                child: DataTable(
                  ...
                  rows: rows.asMap().entries.map((entry) {
                    ...
                    return DataRow(
                      cells: [
                        DataCell(
                          SizedBox(width: idWidth, child: Text('$displayId')),
                        ),
                        DataCell(
                          SizedBox(
                            width: actionWidth,
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: Text(
                              '${row.$4} ${row.$5}${row.$6.trim().isEmpty ? '' : ' | ${row.$6}'}',
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                              style: TextStyle(
                          ...
                          _responsiveFields([
                            ...
                            DropdownButtonFormField<int>(
                              ...
                              items: _roleGroups
                                  ...
                                  .toList(),
                              onChanged: (value) =>
                                  setState(() => _selectedRoleGroupId = value),
                            ),
                          ]),
                        ],
                      );
  ...
  Widget _vehicleCard() => Column(
    ...
    children: [
      ...
      _vehicleRow(
        ...
        carType1,
        carOilType1,
        (v) => setState(() => carType1 = v),
        (v) => setState(() => carOilType1 = v),
      ),
      const SizedBox(height: 12),
      _vehicleRow(
        2,
        ...
        carType2,
        carOilType2,
        (v) => setState(() => carType2 = v),
        (v) => setState(() => carOilType2 = v),
      ),
    ],
  );
  ...
  Widget _vehicleRow(
    ...
    ValueChanged<String?> onOil,
  ) => _responsiveFields(
    [
      _actionField('ทะเบียนรถ', id, width: double.infinity),
      _actionField('สีรถ', color, width: double.infinity),
      DropdownButtonFormField<String>(
        ...
        items: carTypes
            .map(
              (x) => DropdownMenuItem(
                value: _value(x['code']),
                child: LaooComboBoxText(_value(x['name'])),
              ),
            )
            .toList(),
        onChanged: onType,
      ...
      DropdownButtonFormField<String>(
        isExpanded: true,
        initialValue: oil,
        decoration: const InputDecoration(labelText: 'เชื้อเพลิง'),
        items: oilTypes
            .map(
              (x) => DropdownMenuItem(
                value: _value(x['code']),
        ...
        children: [
          ...
          Row(
            ...
            children: [
              IconButton(
                tooltip: 'เลือกรูปรถ',
                onPressed: _carImageBusy ? null : () => _pickCarImage(carNo),
              ...
              IconButton(
                tooltip: 'ลบรูปรถ',
                onPressed: bytes == null ? null : () => _removeCarImage(carNo),
                icon: Icon(
                  Icons.delete_outline,
                  size: 18,
                  color: Theme.of(context).colorScheme.error,
  ...
  Widget _emergencyRow(
    ...
    _emergencyField('ผู้ติดต่อ', name),
    _emergencyField('ความสัมพันธ์', relation),
    _emergencyField('โทรศัพท์', phone),
  ]);
  ...
  Widget _notificationMethodsField() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text(
        'วิธีแจ้งเตือน',
        style: TextStyle(
          fontSize: LaooTypography.subsectionTitle,
          fontWeight: FontWeight.w700,
          color: LaooColors.pageCaption,
        ),
      ),
      ...
      Wrap(
        ...
        children: [
          ...
          _notificationMethodOption(
            ...
            value: notifyBySystem,
            onChanged: (value) => setState(() => notifyBySystem = value),
          ),
        ],
      ),
    ],
  ...
  Widget _notificationMethodOption({
    ...
    child: CheckboxListTile(
      ...
      title: Row(
        children: [
          ...
          Text(label, style: const TextStyle(fontSize: LaooTypography.body)),
        ],
      ),
    ),
  );
  ...
  Future<void> _saveEmployee() async {
    ...
    if (email.isNotEmpty && !_isValidEmail(email)) {
      if (mounted) {
        setState(() {
          _alertMessage = 'คุณกำหนดรูปแบบ email ไม่ถูกต้อง';
          _alertIsError = true;
        });
      }
      return;
    }
    if (!notifyByEmail && !notifyBySystem) {
      setState(() {
        _alertMessage = 'กรุณาเลือกวิธีแจ้งเตือนอย่างน้อย 1 วิธี';
        _alertIsError = true;
      });
      return;
    }
    if (notifyByEmail && email.isEmpty) {
      setState(() {
        _alertMessage = 'กรุณาระบุ Email เมื่อเลือกแจ้งเตือนทาง Email';
        _alertIsError = true;
      });
      return;
    }
    ...
    if (username.isNotEmpty && _selectedRoleGroupId == null) {
      if (mounted) {
        setState(() {
          _alertMessage = 'กรุณาเลือกกลุ่มสิทธิ์';
          _alertIsError = true;
        });
      }
      return;
    ...
    carId1Controller.clear();
    carColor1Controller.clear();
    carId2Controller.clear();
    carColor2Controller.clear();
    _formalImageBytes = null;
    _formalImageName = null;
    _carImage1Bytes = null;
    _carImage2Bytes = null;
  ...
  Future<void> _openEmployeeEdit(String value) async {
    ...
    if (item.isEmpty || !mounted || requestToken != _editRequestToken) {
      return;
    }
    setState(() {
      editingEmployeeId = id;
      ...
      notifyByEmail = item['notifyByEmail'] == true;
      notifyBySystem = item['notifyBySystem'] != false;
      ...
      carType1 = _value(item['carTypeCode1']).isEmpty
          ? null
          : _value(item['carTypeCode1']);
      carOilType1 = _value(item['carOilType1']).isEmpty
          ? null
          : _value(item['carOilType1']);
      carId2Controller.text = _value(item['carID2']);
      carColor2Controller.text = _value(item['carColor2']);
      carType2 = _value(item['carTypeCode2']).isEmpty
          ? null
          : _value(item['carTypeCode2']);
      carOilType2 = _value(item['carOilType2']).isEmpty
          ? null
          : _value(item['carOilType2']);
    ...
    try {
      ...
      if (mounted && requestToken == _editRequestToken) {
        ...
        setState(
          () => _selectedRoleGroupId = (user['roleGroupId'] as num?)?.toInt(),
        );
      }
    } on ApiException {
      usernameController.clear();
  ...
  Future<void> _openEmployeeUserForm(String value, String employeeName) async {
    ...
    InputDecoration userFieldDecoration({
      ...
      return InputDecoration(
        ...
        helperStyle: const TextStyle(fontSize: LaooTypography.inputHint),
        prefixIcon: Icon(icon, color: preset.primary),
              ...
              children: [
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: userFieldDecoration(
                            ...
                            helperText: hasExistingUser
                                ? 'เว้นค่าเดิมไว้ หากไม่ต้องการเปลี่ยนรหัสผ่าน'
                                : null,
                            suffixIcon: IconButton(
                        ...
                        DropdownButtonFormField<int>(
                          ...
                          items: roleGroups
                              .where((e) => e.isActive)
                              .map(
                                (RoleGroup group) => DropdownMenuItem<int>(
                                  value: group.id,
                                  child: Text(
                                    group.name,
                                    style: const TextStyle(
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: const Border(
                      top: BorderSide(color: LaooColors.border),
                    ),
                  ),
                  child: Wrap(
                    spacing: 8,
