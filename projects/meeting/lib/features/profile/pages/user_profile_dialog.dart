...
final ValueNotifier<String> userDefaultViewModeNotifier = ValueNotifier('LIST');
...
class _UserProfileThemeLoaderState extends State<UserProfileThemeLoader> {
  ...
  Future<void> _loadForCurrentUser() async {
    ...
    try {
      ...
      final defaultViewMode = profile['defaultViewMode']
          ?.toString()
          .toUpperCase();
      ...
      if (_loadedUser == requestIdentity) {
        ...
        userDefaultViewModeNotifier.value = defaultViewMode == 'CARD'
            ? 'CARD'
            : 'LIST';
      }
    } catch (_) {
...
class _UserProfileDialogState extends State<_UserProfileDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      userDefaultViewModeNotifier.value = _defaultViewMode;
    ...
    children: [
      ...
      SegmentedButton<String>(
        ...
        onSelectionChanged: (selected) =>
            setState(() => _defaultViewMode = selected.first),
      ),
    ],
  );
    ...
    return AlertDialog(
      title: Row(
        children: [
          ...
          Text('ข้อมูลส่วนตัว', style: LaooTypography.popupTitleStyle),
        ],
      ),
      content: SizedBox(
        width: 520,
