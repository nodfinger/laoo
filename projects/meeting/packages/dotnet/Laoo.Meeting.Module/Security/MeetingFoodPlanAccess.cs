...
""";
    ...
    internal static async Task<bool> Allowed(SqlConnection connection, ClaimsPrincipal principal, string action, CancellationToken token, string screenCode = "21005")
    {
        ...
        const string sql = """
DECLARE @meetingProject bigint = (SELECT ProjectID FROM dbo.TDADProject WHERE ProjectCode='LAOO_MEETING' AND IsActive=1);
SELECT CASE WHEN EXISTS
 ...
 AND EXISTS (SELECT 1 FROM dbo.TDADUserProject WHERE UserID=@user AND CompanyID=@company AND ProjectID=@meetingProject AND IsActive=1)
 AND (
 ...
 (SELECT 1 FROM dbo.TDADUserPermission UP
  ...
  WHERE UP.UserID=@user AND UP.ProjectID=@meetingProject AND UP.IsAllowed=1 AND UP.IsActive=1
 ...
 (SELECT 1 FROM dbo.TDADUserEmployee UE
  ...
  INNER JOIN dbo.TDADRoleGroup RG ON RG.RoleGroupID=ERG.RoleGroupID AND RG.ScopeType='C'
    AND RG.CompanyID=@company AND RG.IsActive=1
  INNER JOIN dbo.TDADRoleGroupPermission RP ON RP.RoleGroupID=RG.RoleGroupID AND RP.ProjectID=@meetingProject
  ...
  WHERE UE.UserID=@user AND UE.CompanyID=@company AND UE.IsActive=1
    AND (RG.ProjectID=RP.ProjectID OR EXISTS
      (SELECT 1 FROM dbo.TDADProject Core WHERE Core.ProjectID=RG.ProjectID AND Core.ProjectCode='LAOO' AND Core.IsActive=1))
