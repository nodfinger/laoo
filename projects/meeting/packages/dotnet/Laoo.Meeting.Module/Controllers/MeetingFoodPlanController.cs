...
OFFSET @skip ROWS FETCH NEXT @take ROWS ONLY;";
        ...
        while (await reader.ReadAsync(token)) items.Add(new
        {
            ...
            canManageFoodPlan = MeetingFoodPlanAccess.CanManage("APPROVED", reader.GetDateTime(6), true, reader.IsDBNull(7) ? canCreate : canEdit),
            canDeleteFoodPlan = !reader.IsDBNull(7) && MeetingFoodPlanAccess.CanManage("APPROVED", reader.GetDateTime(6), true, canDelete),
        });
...
ORDER BY ISNULL(T.Seq,0),F.FoodCode;";
    ...
    public async Task<IActionResult> Save(long bookingId, FoodPlanRequest request, CancellationToken token)
    {
        ...
        await using var connection = await Open(token);
        var exists = await PlanExists(connection, bookingId, company, token);
        if (!await Allowed(connection, "VIEW", token) || !await Allowed(connection, exists ? "EDIT" : "CREATE", token)) return Forbid();
        var header = await BookingHeader(connection, bookingId, company, user, token);
        if (header is null) return Forbid();
...
DELETE FROM dbo.TDADMeetingBookingFoodOption WHERE BookingID=@booking AND CompanyID=@company;";
    ...
    public async Task<IActionResult> Delete(long bookingId, CancellationToken token)
    {
        if (!Scope(out var company, out var user)) return Forbid();
        await using var connection = await Open(token);
        if (!await Allowed(connection, "VIEW", token) || !await Allowed(connection, "DELETE", token)) return Forbid();
        await using var transaction = (SqlTransaction)await connection.BeginTransactionAsync(IsolationLevel.Serializable, token);
        ...
        await using var command = new SqlCommand("DELETE FROM dbo.TDADMeetingBookingFoodOption WHERE BookingID=@booking AND CompanyID=@company; DELETE FROM dbo.TDADMeetingBookingFoodPlan WHERE BookingID=@booking AND CompanyID=@company; SELECT @@ROWCOUNT;", connection, transaction);
        Add(command, "@booking", bookingId); Add(command, "@company", company);
