...
WHERE D.CompanyID=@company AND (@status IS NULL OR D.StatusCode=@status) AND (@room IS NULL OR B.RoomID=@room)
 ...
 AND (@admin=1 OR H.RequestedByUserID=@user
   OR B.RequesterUserID=@user OR EXISTS(SELECT 1 FROM dbo.TDADMeetingRoomContact C JOIN dbo.TDADUserEmployee UE ON UE.EmployeeID=C.EmployeeID AND UE.CompanyID=C.CompanyID AND UE.UserID=@user AND UE.IsActive=1 WHERE C.CompanyID=B.CompanyID AND C.RoomID=B.RoomID AND C.IsActive=1)
   OR (D.StatusCode<>'WAITING_REVIEW' AND EXISTS(SELECT 1 FROM dbo.TDADUserEmployee UE JOIN dbo.TDADEmployee E ON E.EmployeeID=UE.EmployeeID AND E.CompanyID=UE.CompanyID AND E.IsActive=1 WHERE UE.CompanyID=@company AND UE.UserID=@user AND UE.IsActive=1 AND E.DepartmentOrgUnitID=D.ResponsibleDepartmentOrgUnitID)))
...
""";
    ...
    public async Task<IActionResult> Booking(long bookingId, CancellationToken token)
    {
        ...
        var details = await BookingDetails(db, company, user, bookingId, access.CanReview, token);
        return Ok(new { access.BookingId,access.BookingNo,access.Subject,access.RoomCode,access.RoomName,access.Start,access.End,access.Cutoff,access.Active,canConfigurePlan=access.CanReview,canReview=access.CanReview,canRequest=state=="OPEN",requestState=state,items,requests=details });
    }
    ...
    public async Task<IActionResult> Create(long bookingId, EquipmentRequestCreate request, CancellationToken token)
    {
        ...
        var requiresReview = await RequiresReview(db, company, token);
    ...
    public async Task<IActionResult> Cancel(long detailId, CancellationToken token)
    {
        ...
        const string sql = """UPDATE D SET StatusCode='CANCELLED',UpdateBy=@user,UpdateDate=SYSUTCDATETIME() FROM dbo.TDADMeetingBookingEquipmentRequestDetail D JOIN dbo.TDADMeetingBookingEquipmentRequest H ON H.EquipmentRequestID=D.EquipmentRequestID JOIN dbo.TDADMeetingBookingEquipmentPlan P ON P.BookingID=H.BookingID AND P.CompanyID=H.CompanyID WHERE D.CompanyID=@company AND D.EquipmentRequestDetailID=@detail AND (@manager=1 OR H.RequestedByUserID=@user) AND D.StatusCode IN('WAITING_REVIEW','PENDING') AND P.IsActive=1 AND P.RequestCutoffDateTime>SYSDATETIME();""";
        await using var cmd = new SqlCommand(sql, db);
        Add(cmd, "@user", user); Add(cmd, "@company", company); Add(cmd, "@detail", detailId); Add(cmd, "@manager", manager);
        if (await cmd.ExecuteNonQueryAsync(token) == 0) return BadRequest(Error("ยกเลิกรายการไม่ได้", "ยกเลิกได้เฉพาะผู้ขอหรือผู้ดูแลการประชุม สำหรับรายการที่ยังรอตรวจสอบหรือรอดำเนินการภายในเวลาปิดรับ"));
        return NoContent();
    }
    ...
    public async Task<IActionResult> Status(long detailId, EquipmentRequestStatus request, CancellationToken token)
    {
        ...
        await using var cmd=new SqlCommand(sql,db);Add(cmd,"@status",status);Add(cmd,"@remark",Clean(request.ResultRemark));Add(cmd,"@user",user);Add(cmd,"@company",company);Add(cmd,"@detail",detailId);Add(cmd,"@manager",manager);Add(cmd,"@edit",edit);if(await cmd.ExecuteNonQueryAsync(token)==0)return Forbid();return NoContent();
    }
    ...
    public async Task<IActionResult> Timeline(long requestId, CancellationToken token)
    {
        ...
        const string sql="SELECT EquipmentRequestDetailID,EventCode,EventRemark,ActorName,CreateDate FROM dbo.TDADMeetingBookingEquipmentRequestTimeline WHERE CompanyID=@company AND EquipmentRequestID=@request ORDER BY CreateDate,EquipmentRequestTimelineID;";await using var cmd=new SqlCommand(sql,db);Add(cmd,"@company",company);Add(cmd,"@request",requestId);await using var r=await cmd.ExecuteReaderAsync(token);var items=new List<object>();while(await r.ReadAsync(token))items.Add(new{detailId=Long(r,0),eventCode=r.GetString(1),remark=Text(r,2),actorName=Text(r,3),createDate=r.GetDateTime(4)});return Ok(new{items});
    }

    private async Task<bool> RequiresReview(SqlConnection db,long company,CancellationToken token){await using var cmd=new SqlCommand("SELECT TOP(1) RequireEquipmentRequestReview FROM dbo.TDSTCompanySetupSystemMeeting WHERE CompanyID=@company AND ProjectID=@project",db);Add(cmd,"@company",company);Add(cmd,"@project",MeetingProjectId);var value=await cmd.ExecuteScalarAsync(token);return value is not null&&value!=DBNull.Value&&Convert.ToBoolean(value);}
