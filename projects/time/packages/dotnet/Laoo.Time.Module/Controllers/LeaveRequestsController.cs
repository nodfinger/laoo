...
""",c);Add(header,"@C",SqlDbType.BigInt,companyId);Add(header,"@R",SqlDbType.BigInt,requestId);
    ...
    public async Task<IActionResult> Create([FromQuery] string mode, LeaveRequestSaveRequest request, CancellationToken token)
    {
        ...
        try
        {
            ...
            if (direct)
            {
                ...
                await RecalculateLeaveDates(c, tx, companyId, employeeId.Value, requestId, userId, token);
            }
            else
            {
        ...
        CancellationToken token)
    {
        ...
        try
        {
            ...
            if(decision=="APPROVED")await RecalculateLeaveDates(c,tx,companyId,pending.EmployeeId,requestId,userId,token);
    ...
    [HttpPost("{requestId:long}/cancel-approved")]
    public async Task<IActionResult> CancelApproved(long requestId, LeaveCancelRequest request, CancellationToken token)
    {
        if (!RowVersion(request.RowVersion) || !Scope(out var companyId, out var userId)) return BadRequest(new { message = "ข้อมูลการยกเลิกคำขอไม่ถูกต้อง" });
        await using var c = await Open(token);
        if (!await Can(c, SelfMenu, "CANCEL", token)) return Forbid();
        var actorEmployee = await EmployeeForUser(c, companyId, userId, token);
        if (!actorEmployee.HasValue) return Conflict(new { message = "บัญชีผู้ใช้ยังไม่ได้ผูกกับพนักงานที่ใช้งานอยู่" });
        await using var tx = (SqlTransaction)await c.BeginTransactionAsync(IsolationLevel.Serializable, token);
        try
        {
            await using var owner = new SqlCommand("SELECT SubjectEmployeeID FROM dbo.TDTMRequest WITH(UPDLOCK,HOLDLOCK) WHERE CompanyID=@C AND RequestID=@R AND ProcessCode='LEAVE_REQUEST' AND StatusCode='APPROVED' AND ActorUserID=@U", c, tx);
            Add(owner, "@C", SqlDbType.BigInt, companyId); Add(owner, "@R", SqlDbType.BigInt, requestId); Add(owner, "@U", SqlDbType.BigInt, userId);
            if (Convert.ToInt64(await owner.ExecuteScalarAsync(token) ?? 0) != actorEmployee.Value) throw new InvalidOperationException("ยกเลิกได้เฉพาะคำขอลาที่อนุมัติแล้วของตนเอง");
            var usages = await UsageForRequest(c, tx, requestId, token);
            if (usages.Count == 0) throw new InvalidOperationException("ไม่พบรายการใช้สิทธิ์ที่คืนได้สำหรับคำขอนี้");
            foreach (var usage in usages)
                await Ledger(c, tx, companyId, actorEmployee.Value, new Type(usage.LeaveTypeId, usage.UnitCode, false, false), requestId, "CANCEL_RELEASE", usage.Quantity, userId, token);
            await using var update = new SqlCommand("UPDATE dbo.TDTMRequest SET StatusCode='CANCELLED',UpdateDate=SYSDATETIME(),UpdateBy=@U WHERE CompanyID=@C AND RequestID=@R AND StatusCode='APPROVED' AND RowVersion=CONVERT(binary(8),@V,2)", c, tx);
            Add(update, "@U", SqlDbType.BigInt, userId); Add(update, "@C", SqlDbType.BigInt, companyId); Add(update, "@R", SqlDbType.BigInt, requestId); Add(update, "@V", SqlDbType.VarChar, request.RowVersion, 32);
            if (await update.ExecuteNonQueryAsync(token) != 1) throw new InvalidOperationException("คำขอถูกแก้ไขแล้ว กรุณาโหลดใหม่");
            await using var audit = new SqlCommand("INSERT dbo.TDTMApprovalDecision(RequestID,StepOrder,DecisionCode,ActorUserID,ActorEmployeeID,Reason,CorrelationID)VALUES(@R,0,'CANCELLED',@U,@E,@Reason,NEWID())", c, tx);
            Add(audit, "@R", SqlDbType.BigInt, requestId); Add(audit, "@U", SqlDbType.BigInt, userId); Add(audit, "@E", SqlDbType.BigInt, actorEmployee); Add(audit, "@Reason", SqlDbType.NVarChar, request.Reason, 1000); await audit.ExecuteNonQueryAsync(token);
            await RecalculateLeaveDates(c, tx, companyId, actorEmployee.Value, requestId, userId, token);
            await Notify(c, tx, companyId, requestId, actorEmployee.Value, "CANCELLED", token);
            await tx.CommitAsync(token); return NoContent();
        }
        catch (InvalidOperationException e) { await tx.RollbackAsync(token); return Conflict(new { message = e.Message }); }
    ...
    private sealed record Policy(long Id,string Code); private sealed record Profile(long BaseId,long? ProcessId,string Code); private sealed record Type(long Id,string Unit,bool RequireRemark,bool RequireEvidence); private sealed record Allocation(long Id,DateOnly WorkDate,decimal Quantity); private sealed record PendingRequest(long EmployeeId); private sealed record Reserved(long LeaveTypeId,string UnitCode,decimal Quantity); private sealed record UsedLeave(long LeaveTypeId,string UnitCode,decimal Quantity);
...
""",c,tx);Add(q,"@C",SqlDbType.BigInt,company);Add(q,"@U",SqlDbType.BigInt,user);Add(q,"@E",SqlDbType.BigInt,employee);return Convert.ToBoolean(await q.ExecuteScalarAsync(t));}
    ...
    private static async Task<List<UsedLeave>> UsageForRequest(SqlConnection c,SqlTransaction tx,long request,CancellationToken t){await using var q=new SqlCommand("SELECT LeaveTypeID,UnitCode,UsedQuantity FROM dbo.TDTMLeaveUsage WITH(UPDLOCK,HOLDLOCK) WHERE RequestID=@R",c,tx);Add(q,"@R",SqlDbType.BigInt,request);await using var r=await q.ExecuteReaderAsync(t);var rows=new List<UsedLeave>();while(await r.ReadAsync(t))rows.Add(new UsedLeave(r.GetInt64(0),r.GetString(1),r.GetDecimal(2)));return rows;}
    ...
    private static async Task RecalculateLeaveDates(SqlConnection c,SqlTransaction tx,long company,long employee,long request,long actor,CancellationToken t){await using var q=new SqlCommand("SELECT WorkDate FROM dbo.TDTMLeaveRequestDate WHERE RequestID=@R ORDER BY WorkDate",c,tx);Add(q,"@R",SqlDbType.BigInt,request);await using var r=await q.ExecuteReaderAsync(t);var dates=new List<DateOnly>();while(await r.ReadAsync(t))dates.Add(DateOnly.FromDateTime(r.GetDateTime(0)));await r.CloseAsync();foreach(var date in dates)await AttendanceCalculator.RecalculateAsync(c,tx,company,employee,date,actor,t);}
