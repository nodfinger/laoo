...
public static class AttendanceCalculator
{
        ...
        long companyId, long employeeId, DateOnly workDate, long actorId, CancellationToken token)
    {
        ...
        var leaveRows = await ApprovedLeaveRows(connection, transaction, companyId, employeeId, workDate, token);
        var leaveMinutes = leaveRows.Sum(x => x.UnitCode == "DAY" ? Math.Max(0, scheduled) : (int)x.Quantity);
        var status = leaveMinutes >= Math.Max(0, scheduled) && scheduled > 0 ? "LEAVE" : leaveMinutes > 0 && unresolved ? "LEAVE_PARTIAL" : unresolved ? "UNRESOLVED" : "COMPLETE";
        var resultId = await InsertResult(connection, transaction, companyId, employeeId, workDate, shiftVersionId, resultVersion,
            status, Math.Max(0, scheduled), rows.Sum(x => x.Work), rows.Sum(x => x.Late), rows.Sum(x => x.Early), status.StartsWith("LEAVE", StringComparison.Ordinal) ? null : reason, actorId, rows, token);
        foreach (var leave in leaveRows)
            await ApplyLeave(connection, transaction, companyId, resultId, leave.RequestId, leave.DateId, leave.UnitCode == "DAY" ? Math.Max(0, scheduled) : (int)leave.Quantity, actorId, token);
    }
...
WITH EffectiveShift AS(
 ...
 OUTER APPLY(SELECT TOP(1) HomeBranchID FROM dbo.TDADEmployeeOrganizationAssignment WHERE CompanyID=@C AND EmployeeID=@E AND IsActive=1 AND EffectiveFrom<=@D AND(EffectiveTo IS NULL OR EffectiveTo>=@D) ORDER BY EffectiveFrom DESC,EmployeeOrganizationAssignmentID DESC)HB
 OUTER APPLY(SELECT TOP(1) BranchHolidayExceptionID,IsHoliday FROM dbo.TDTMBranchHolidayException WHERE CompanyID=@C AND BranchID=HB.HomeBranchID AND HolidayDate=@D AND IsActive=1 ORDER BY BranchHolidayExceptionID DESC)HE
 OUTER APPLY(SELECT TOP(1) HolidayCalendarID FROM dbo.TDTMBranchHolidayCalendarAssignment WHERE CompanyID=@C AND BranchID=HB.HomeBranchID AND IsActive=1 AND EffectiveFrom<=@D AND(EffectiveTo IS NULL OR EffectiveTo>=@D) ORDER BY EffectiveFrom DESC,BranchHolidayCalendarAssignmentID DESC)BH
 OUTER APPLY(SELECT TOP(1) HolidayCalendarID FROM dbo.TDTMDefaultHolidayCalendarVersion WHERE CompanyID=@C AND IsActive=1 AND EffectiveFrom<=@D AND(EffectiveTo IS NULL OR EffectiveTo>=@D) ORDER BY EffectiveFrom DESC,DefaultHolidayCalendarVersionID DESC)DH
 OUTER APPLY(SELECT TOP(1) HolidayDateID FROM dbo.TDTMHolidayDate WHERE CompanyID=@C AND HolidayCalendarID=COALESCE(BH.HolidayCalendarID,DH.HolidayCalendarID) AND HolidayDate=@D AND IsActive=1)HD
 ...
 WHERE ISNULL(O.IsDayOff,CASE WHEN HE.BranchHolidayExceptionID IS NOT NULL THEN HE.IsHoliday WHEN HD.HolidayDateID IS NOT NULL THEN 1 ELSE ISNULL(RD.IsDayOff,1) END)=0), V AS(
...
""";
    ...
    private sealed record LeaveRow(long RequestId,long DateId,string UnitCode,decimal Quantity);
    private static async Task<List<LeaveRow>> ApprovedLeaveRows(SqlConnection c,SqlTransaction tx,long company,long employee,DateOnly date,CancellationToken token){await using var q=new SqlCommand("SELECT R.RequestID,D.LeaveRequestDateID,L.UnitCode,D.RequestedQuantity FROM dbo.TDTMRequest R JOIN dbo.TDTMLeaveRequest L ON L.RequestID=R.RequestID JOIN dbo.TDTMLeaveRequestDate D ON D.RequestID=R.RequestID WHERE R.CompanyID=@C AND R.SubjectEmployeeID=@E AND R.ProcessCode='LEAVE_REQUEST' AND R.StatusCode='APPROVED' AND D.WorkDate=@D",c,tx);Add(q,"@C",SqlDbType.BigInt,company);Add(q,"@E",SqlDbType.BigInt,employee);Add(q,"@D",SqlDbType.Date,date.ToDateTime(TimeOnly.MinValue));await using var r=await q.ExecuteReaderAsync(token);var rows=new List<LeaveRow>();while(await r.ReadAsync(token))rows.Add(new LeaveRow(r.GetInt64(0),r.GetInt64(1),r.GetString(2),r.GetDecimal(3)));return rows;}
    private static async Task ApplyLeave(SqlConnection c,SqlTransaction tx,long company,long result,long request,long date,int minutes,long actor,CancellationToken token){if(minutes<=0)return;await using var q=new SqlCommand("INSERT dbo.TDTMAttendanceLeaveApplication(CompanyID,AttendanceResultID,LeaveRequestID,LeaveRequestDateID,AppliedLeaveMinutes,CreateBy)VALUES(@C,@R,@Q,@D,@M,@U)",c,tx);Add(q,"@C",SqlDbType.BigInt,company);Add(q,"@R",SqlDbType.BigInt,result);Add(q,"@Q",SqlDbType.BigInt,request);Add(q,"@D",SqlDbType.BigInt,date);Add(q,"@M",SqlDbType.Int,minutes);Add(q,"@U",SqlDbType.BigInt,actor);await q.ExecuteNonQueryAsync(token);}
    private static async Task<long> InsertResult(SqlConnection c,SqlTransaction tx,long company,long employee,DateOnly date,long? version,int resultVersion,string status,int scheduled,int actual,int late,int early,string? reason,long actor,IReadOnlyList<(Session Session,DateTime? In,DateTime? Out,int Work,int Late,int Early)> rows,CancellationToken token){await using var h=new SqlCommand("INSERT dbo.TDTMAttendanceResult(CompanyID,EmployeeID,WorkDate,ShiftTemplateVersionID,ResultVersion,StatusCode,ScheduledWorkMinutes,ActualWorkMinutes,LateMinutes,EarlyMinutes,UnresolvedReason,CreateBy) OUTPUT INSERTED.AttendanceResultID VALUES(@C,@E,@D,@V,@N,@S,@SW,@AW,@L,@ER,@R,@U)",c,tx);Add(h,"@C",SqlDbType.BigInt,company);Add(h,"@E",SqlDbType.BigInt,employee);Add(h,"@D",SqlDbType.Date,date.ToDateTime(TimeOnly.MinValue));Add(h,"@V",SqlDbType.BigInt,version);Add(h,"@N",SqlDbType.Int,resultVersion);Add(h,"@S",SqlDbType.VarChar,status,20);Add(h,"@SW",SqlDbType.Int,scheduled);Add(h,"@AW",SqlDbType.Int,actual);Add(h,"@L",SqlDbType.Int,late);Add(h,"@ER",SqlDbType.Int,early);Add(h,"@R",SqlDbType.NVarChar,reason,1000);Add(h,"@U",SqlDbType.BigInt,actor);var id=Convert.ToInt64(await h.ExecuteScalarAsync(token));foreach(var row in rows){await using var d=new SqlCommand("INSERT dbo.TDTMAttendanceSessionResult(AttendanceResultID,AttendanceSessionRuleID,SequenceNo,SessionStatusCode,ActualInDateTime,ActualOutDateTime,WorkMinutes,LateMinutes,EarlyMinutes)VALUES(@I,@R,@N,@S,@IN,@OUT,@W,@L,@E)",c,tx);Add(d,"@I",SqlDbType.BigInt,id);Add(d,"@R",SqlDbType.BigInt,row.Session.Id);Add(d,"@N",SqlDbType.Int,row.Session.Sequence);Add(d,"@S",SqlDbType.VarChar,row.In.HasValue&&row.Out.HasValue&&row.Out>row.In?"COMPLETE":"UNRESOLVED",20);Add(d,"@IN",SqlDbType.DateTime2,row.In);Add(d,"@OUT",SqlDbType.DateTime2,row.Out);Add(d,"@W",SqlDbType.Int,row.Work);Add(d,"@L",SqlDbType.Int,row.Late);Add(d,"@E",SqlDbType.Int,row.Early);await d.ExecuteNonQueryAsync(token);}return id;}
