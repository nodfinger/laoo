...
class EmployeeTimeSettingRecord {
  ...
  factory EmployeeTimeSettingRecord.fromJson(Map<String, dynamic> json) =>
      EmployeeTimeSettingRecord(
        ...
        divisionOrgUnitId: (json['divisionOrgUnitId'] as num?)?.toInt(),
        divisionName: _text(json['divisionName']),
        departmentOrgUnitId: (json['departmentOrgUnitId'] as num?)?.toInt(),
...
class EmployeeTimeSettingsDetail {
  const EmployeeTimeSettingsDetail({
    required this.employee,
    required this.businessDate,
    required this.requirementHistory,
    required this.deviceCodeHistory,
    required this.auditHistory,
  });

  final EmployeeTimeSettingRecord employee;
  final DateTime businessDate;
  final List<AttendanceRequirementVersion> requirementHistory;
  final List<DeviceCodeVersion> deviceCodeHistory;
  final List<EmployeeTimeSettingAudit> auditHistory;

  factory EmployeeTimeSettingsDetail.fromJson(Map<String, dynamic> json) {
    final employeeJson = Map<String, dynamic>.from(json['employee'] as Map);
    return EmployeeTimeSettingsDetail(
      employee: EmployeeTimeSettingRecord.fromJson(employeeJson),
      businessDate: _date(employeeJson['businessDate']) ?? DateTime(1970, 1, 1),
      requirementHistory: _mapList(
        json['requirementHistory'],
        AttendanceRequirementVersion.fromJson,
      ),
      deviceCodeHistory: _mapList(
        json['deviceCodeHistory'],
        DeviceCodeVersion.fromJson,
      ),
      auditHistory: _mapList(
        json['auditHistory'],
        EmployeeTimeSettingAudit.fromJson,
      ),
    );
  }
}

class AttendanceRequirementVersion {
  const AttendanceRequirementVersion({
    required this.requirementCode,
    required this.effectiveFrom,
    required this.effectiveTo,
    required this.reason,
    required this.supersededUtc,
  });
  ...
  factory AttendanceRequirementVersion.fromJson(Map<String, dynamic> json) =>
      AttendanceRequirementVersion(
        requirementCode: json['requirementCode']?.toString() ?? 'REQUIRED',
        effectiveFrom: _date(json['effectiveFrom']),
        effectiveTo: _date(json['effectiveTo']),
        reason: _text(json['reason']),
        supersededUtc: _date(json['supersededUtc']),
      );
}

class DeviceCodeVersion {
  const DeviceCodeVersion({
    required this.deviceCode,
    required this.effectiveFrom,
    required this.effectiveTo,
    required this.supersededUtc,
  ...
  factory DeviceCodeVersion.fromJson(Map<String, dynamic> json) =>
      DeviceCodeVersion(
...
class EmployeeTimeSettingAudit {
  const EmployeeTimeSettingAudit({
    required this.reason,
    required this.actorUserId,
    required this.actorName,
    required this.occurredDateUtc,
  });

  final String reason;
  final int actorUserId;
  final String? actorName;
  final DateTime? occurredDateUtc;

  factory EmployeeTimeSettingAudit.fromJson(Map<String, dynamic> json) =>
      EmployeeTimeSettingAudit(
        reason: json['reason']?.toString() ?? '',
        actorUserId: (json['actorUserId'] as num?)?.toInt() ?? 0,
        actorName: _text(json['actorName']),
        occurredDateUtc: _utcDate(json['occurredDateUtc']),
      );
}
