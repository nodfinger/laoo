/* Core-owned navigation and permission bootstrap for LAOO_TIME attendance summary report. */
SET NOCOUNT ON;
SET XACT_ABORT ON;
BEGIN TRY
BEGIN TRANSACTION;
...
IF @TimeProjectID IS NULL THROW 52810,N'Active LAOO_TIME project is required.',1;
...
INSERT @Menus VALUES
(N'29004',N'29',NCHAR(3619)+NCHAR(3634)+NCHAR(3618)+NCHAR(3591)+NCHAR(3634)+NCHAR(3609)+NCHAR(3626)+NCHAR(3619)+NCHAR(3640)+NCHAR(3611)+NCHAR(3612)+NCHAR(3621)+NCHAR(3621)+NCHAR(3591)+NCHAR(3648)+NCHAR(3623)+NCHAR(3621)+NCHAR(3634),3,N'timeAttendanceSummaryReport',N'/company/time-attendance-summary-report',N'TIME_ATTENDANCE_SUMMARY_REPORT',N'analytics_outlined',40);

IF EXISTS(SELECT 1 FROM @Menus S JOIN dbo.TDADMainMenu T ON T.MenuCode=S.MenuCode WHERE T.ScreenType<>S.ScreenType OR ISNULL(T.RouteName,N'')<>S.RouteName OR ISNULL(T.RoutePath,N'')<>S.RoutePath) THROW 52811,N'Time attendance summary report MenuCode conflicts with ScreenType or route.',1;
IF EXISTS(SELECT 1 FROM @Menus S JOIN dbo.TDADMainMenu T ON (T.RouteName=S.RouteName OR T.RoutePath=S.RoutePath) AND T.MenuCode<>S.MenuCode) THROW 52812,N'Time attendance summary report RouteName or RoutePath is already used.',1;

UPDATE dbo.TDADMenuGroup SET IsActive=1,UpdateDate=SYSUTCDATETIME() WHERE MenuGroupCode=N'29';
IF NOT EXISTS(SELECT 1 FROM dbo.TDADMenuGroup WHERE MenuGroupCode=N'29')
  THROW 52813,N'Menu group 29 must be bootstrapped before the attendance summary report.',1;
...
UPDATE dbo.TDADProjectMenuGroup SET IsActive=1,UpdateDate=SYSUTCDATETIME() WHERE ProjectID=@TimeProjectID AND MenuGroupCode=N'29';
IF NOT EXISTS(SELECT 1 FROM dbo.TDADProjectMenuGroup WHERE ProjectID=@TimeProjectID AND MenuGroupCode=N'29')
  INSERT dbo.TDADProjectMenuGroup(ProjectID,MenuGroupCode,SortOrder,IsActive,CreateDate) VALUES(@TimeProjectID,N'29',5,1,SYSUTCDATETIME());
...
UPDATE dbo.TDADProjectMenu SET IsActive=0,UpdateDate=SYSUTCDATETIME() WHERE MenuCode=N'29004' AND ProjectID<>@TimeProjectID AND IsActive=1;
...
(N'29004',NCHAR(3619)+NCHAR(3634)+NCHAR(3618)+NCHAR(3591)+NCHAR(3634)+NCHAR(3609)+NCHAR(3626)+NCHAR(3619)+NCHAR(3640)+NCHAR(3611)+NCHAR(3612)+NCHAR(3621)+NCHAR(3621)+NCHAR(3591)+NCHAR(3648)+NCHAR(3623)+NCHAR(3621)+NCHAR(3634),N'Attendance summary report',N'VIEW',NCHAR(3604)+NCHAR(3641)+NCHAR(3586)+NCHAR(3657)+NCHAR(3629)+NCHAR(3617)+NCHAR(3641)+NCHAR(3621),N'View');
...
UPDATE T SET T.IsActive=0 FROM dbo.TDADPermission T LEFT JOIN @Permissions S ON S.ScreenCode=T.ScreenCode AND S.ActionCode=T.ActionCode WHERE T.ProjectID=@TimeProjectID AND T.ScreenCode=N'29004' AND S.ScreenCode IS NULL;
