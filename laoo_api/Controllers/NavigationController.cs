...
""";
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            if (groupCodes.Contains(group.MenuGroupCode) && (admin || item.FeatureCode is null || allowedFeatures!.Contains(item.FeatureCode))) all.Add((group, item));
        }
    ...
    {
        if (userType == "LAOO_SUPPORT") return true;
        var sql = userType == "PARTNER_USER"
            ? "SELECT CASE WHEN EXISTS (SELECT 1 FROM dbo.TDADPartnerUser WHERE PartnerID=@PartnerID AND IsPartnerAdmin=1 AND IsActive=1) THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END"
    ...
    private async Task<HashSet<string>> LoadAllowedFeaturesAsync(SqlConnection connection, string userType, CancellationToken token)
    {
        ...
        var sql = userType == "PARTNER_USER" ? $"""
SELECT P.ScreenCode FROM dbo.TDADPartnerUserPermission UP INNER JOIN dbo.TDADPermission P ON P.PermissionID=UP.PermissionID AND P.ProjectID=UP.ProjectID INNER JOIN dbo.TDADPartnerUser U ON U.PartnerUserID=UP.PartnerUserID WHERE U.NormalizedUsername=@Username AND UP.IsAllowed=1 AND UP.IsActive=1 AND P.IsActive=1 AND P.ActionCode='VIEW';
""" : $"""
SELECT P.ScreenCode FROM dbo.{permissionTable} UP INNER JOIN dbo.TDADPermission P ON P.PermissionID=UP.PermissionID AND P.ProjectID=UP.ProjectID WHERE UP.{idColumn}=@ID AND UP.IsAllowed=1 AND UP.IsActive=1 AND P.IsActive=1 AND P.ActionCode='VIEW';
""";
        await using var command = new SqlCommand(sql, connection);
