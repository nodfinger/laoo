...
public sealed class BranchController(IConfiguration configuration) : ControllerBase
{
    ...
    private async Task<IActionResult> Save(long? id, BranchRequest x, CancellationToken token)
    {
        ...
        if (IsPartner())
        {
            await using var access = new SqlCommand("SELECT CASE WHEN EXISTS (SELECT 1 FROM dbo.TDADCompany WHERE CompanyID=@company AND PartnerID=@partnerId) AND (@id IS NULL OR EXISTS (SELECT 1 FROM dbo.TDADBranch B INNER JOIN dbo.TDADCompany C ON C.CompanyID=B.CompanyID WHERE B.BranchID=@id AND C.PartnerID=@partnerId)) THEN 1 ELSE 0 END", c);
