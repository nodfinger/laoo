...
public sealed class MeetingRoomRuleController(IConfiguration configuration) : ControllerBase
{
    ...
    private Task<bool> Allowed(SqlConnection c, string action, CancellationToken token) => LaooApi.Security.CompanyProjectPermission.IsAllowedAsync(c, User, ScreenCode, action, token);
