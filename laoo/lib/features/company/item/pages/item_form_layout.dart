...
class _ItemFormLayoutState extends State<ItemFormLayout> {
  ...
  late final TextEditingController _remarkItem1, _note1, _note2, _note3, _note4, _note5;
  ...
  Future<void> _pickImage() async {
    ...
    if (prepared.bytes.length > 1024 * 1024) {
      if (mounted) {
        showTimedSnackBar(
          context,
          message: 'ไม่สามารถลดขนาดรูปภาพให้เหลือไม่เกิน 1 MB ได้',
          error: true,
        );
      }
      return;
  ...
  ({List<int> bytes, String contentType}) _prepareImage(List<int> source, String? extension) {
    if (source.length <= 1024 * 1024) {
      return (bytes: source, contentType: extension?.toLowerCase() == 'png' ? 'image/png' : 'image/jpeg');
    }
    ...
    for (var pass = 0; pass < 4; pass++) {
      ...
      for (var quality = 85; quality >= 35; quality -= 10) {
        ...
        if (bytes.length <= 1024 * 1024) return (bytes: bytes, contentType: 'image/jpeg');
      }
      current = img.copyResize(current, width: (current.width * .8).round());
    }
    return (bytes: img.encodeJpg(current, quality: 30), contentType: 'image/jpeg');
  }
              ...
              children: [
                FittedBox(
                  ...
                  child: SizedBox(
                    ...
                    child: Row(
                  children: [
                    Expanded(child: WorkspacePageTitle(title: title)),
  ...
  Widget _compactFields(bool wide) {
    ...
    final code = _text(_code, 'รหัสสินค้า', required: true);
