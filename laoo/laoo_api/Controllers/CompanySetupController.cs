...
ELSE
    INSERT dbo.TDSTCompanySetupSystem
        (ProjectID,OwnerType,PartnerID,CompanyID,RunItem,ItemDigit,RunCus,CustomerDigit,IsActive,CreateDate)
    VALUES
        (@ProjectID,@OwnerType,@PartnerID,@CompanyID,@RunItem,@ItemDigit,@RunCus,@CustomerDigit,1,SYSUTCDATETIME());
""", connection, transaction))
        ...
        if (response.ItemDigit != request.ItemDigit)
        ...
        if (response.CustomerDigit != request.CustomerDigit ||
