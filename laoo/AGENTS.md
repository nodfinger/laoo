...
- การแสดงข้อมูลแบบ Data List ต้องใช้ความกว้างของพื้นที่จริงจาก `LayoutBuilder` โดยกำหนด `width: constraints.maxWidth` ให้ Container/SizedBox ที่ครอบตารางเสมอ เพื่อให้ตารางเต็มพื้นที่ Content Area และไม่เหลือพื้นที่ว่างด้านข้าง
- Data List ห้ามใช้ Horizontal Scrollbar เป็นค่าเริ่มต้น และต้องปรับขนาดคอลัมน์ให้พอดีกับ `constraints.maxWidth` โดยไม่ล้นหน้าจอ
- เมื่อความกว้าง Content Area ต่ำกว่า `900px` ให้หน้าจอ List เปลี่ยนเป็น Card Mode อัตโนมัติ เพื่อป้องกันตารางล้นและให้ข้อมูลอ่านได้บนมือถือ/หน้าจอแคบ
