...
class TechnicalInfoRepository {
  TechnicalInfoRepository({ApiClient? apiClient})
    : _api = apiClient ?? ApiClient();
  final ApiClient _api;

  Future<TechnicalInfoResult> search(String query) async {
    final data = await _api.get(
      '/api/technical-info/search',
      query: {'q': query},
    );
  ...
  Future<TechnicalFileContent> readFile(String path) async {
    final data = await _api.get(
      '/api/technical-info/file-content',
      query: {'path': path},
    );
...
class TechnicalInfoResult {
  ...
  factory TechnicalInfoResult.fromJson(Map<String, dynamic> json) =>
      TechnicalInfoResult(
        menus: List<Map<String, dynamic>>.from(
          json['menus'] as List? ?? const [],
        ),
        dartFiles: List<Map<String, dynamic>>.from(
          json['dartFiles'] as List? ?? const [],
        ),
        mds: List<Map<String, dynamic>>.from(json['mds'] as List? ?? const []),
        tables: List<Map<String, dynamic>>.from(
          json['tables'] as List? ?? const [],
        ),
      );
}
