...
class MeetingStructureRepository {
  ...
  Future<int> saveBuilding(Map<String, dynamic> body, {int? id}) async =>
      ((await (id == null
                  ? _api.post('$_path/buildings', body: body)
                  : _api.put('$_path/buildings/$id', body: body)))
              as Map)['buildingId']
          as int;
  Future<String> uploadBuildingImage(
    int id,
    List<int> bytes,
    String filename,
  ) async =>
      ((await _api.upload(
                '$_path/buildings/$id/image',
                bytes: bytes,
                filename: filename,
              ))
              as Map)['imageUrl']
          as String;
  Future<void> saveFloor(Map<String, dynamic> body, {int? id}) async =>
      id == null
      ? _api.post('$_path/floors', body: body)
      : _api.put('$_path/floors/$id', body: body);
